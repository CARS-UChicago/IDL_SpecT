...
;       *pixel*.  [ Thus any two dimensional sampling is permitted,
;       including irregular sampling. ]
;
;       If the sampling is regular, then the x coordinates are the
;       same for each row, and the y coordinates are the same for each
;       column.  Call the x-row and y-column coordinates XR and YC
;       respectively.  You can then compute X and Y as follows:
;
;           X = XR # (YC*0 + 1)             eqn. 1
;           Y = (XR*0 + 1) # YC             eqn. 2
;
...
;                                     pixels
;
;       Then using equations 1 and 2 above will give these values to X
;       and Y:
;
...
;              17 17 17 17 17
;
;       Using the above technique is suggested, but *not* required.
;       You can do anything you wish with the X and Y values.  This
;       technique only makes it easier to compute your model function
;       values.
;
;   Y - Array of "Y" indendent variable values.  See discussion under
;       X above.  X and Y should have the same data type.
;
