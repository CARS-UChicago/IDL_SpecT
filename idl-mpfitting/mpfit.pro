...
pro mpfit_resetprof
  common mpfit_profile, prof_fdjac2, prof_lmpar, prof_mpfit, prof_qrfac, $
    prof_qrsolv, prof_enorm
  prof_fdjac2 = 0D
  prof_lmpar = 0D
  prof_mpfit = 0D
  prof_qrfac = 0D
...
pro mpfit_setmachar, double=double
  common mpfit_profile
  if n_elements(prof_fdjac2) EQ 0 then mpfit_resetprof
...
;  prof_start = systime(1)
  ...
  if NOT keyword_set(autoderiv) then begin
      ...
      if n_elements(fjac) NE m*nall then begin
          ...
          iflag = 1
;          prof_fdjac2 = prof_fdjac2 + (systime(1) - prof_start)
          return, 0
      endif
...
; Very simple-minded sum-of-squares
  ...
  dummy = check_math(1, 1)  ;; Avoid harmless underflow messages
  if isdouble then begin
      RDWARF  = sqrt(dminnum) * 10.
      RGIANT  = sqrt(dmaxnum) * 0.1
  endif else begin
      RDWARF  = sqrt(fminnum) * 10.
      RGIANT  = sqrt(fmaxnum) * 0.1
  endelse
  agiant = rgiant / n_elements(vec)
  adwarf = rdwarf * n_elements(vec)
  dummy = check_math(0, 0)
  ...
  mx = max(vec, min=mn) & mx = max(abs([mx,mn]))
  ...
  TERMINATE:
;  prof_enorm = prof_enorm + (systime(1) - prof_start)
  return, ans
end
...
;  prof_qrfac = prof_qrfac + (systime(1) - prof_start)
  return
end
...
;  prof_qrsolv = prof_qrsolv + (systime(1) - prof_start)
  return
end
...
TERMINATE:
  ;; Termination
;  prof_lmpar = prof_lmpar + (systime(1) - prof_start)
...
;  prof_mpfit = prof_mpfit + (systime(1) - prof_start)
